...
def test_download_callable_is_file_backed_not_eager_bytes():
    source = Path("mide/gs496_static_session_backup.py").read_text(encoding="utf-8")
    block_start = source.index("def open_prepared_archive")
    block_end = source.index("st.download_button(", block_start)
    block = source[block_start:block_end]

    assert '.open("rb")' in block
    assert ".read_bytes()" not in block
...
def test_native_download_remains_inside_self_refreshing_fragment():
    ...
    assert "open_prepared_archive" in source
