...
class Response:
    ...
    def json(self): return {"data":[{"symbol":self.symbol,"name":f"{self.symbol} Inc","price":"1.25","change_ratio":"42.0","volume":"2500000","relative_volume_10d":"7.5"}]}
class Screener:
    def get_gainers_losers(self,**kwargs): return Response("GAIN" if kwargs["rank_type"]=="DAY_1" else "M5")
    def get_most_active(self,**kwargs): return Response("RVOL" if kwargs["rank_type"]=="RELATIVE_VOLUME_10D" else "VOL")
class RawDataClient:
    def __init__(self): self.screener=Screener()
class SDKAdapter:
    def __init__(self): self.sdk_client=RawDataClient()
class SnapshotClient:
    def __init__(self): self.sdk=SDKAdapter()
