...
def test_native_radar_uses_official_rank_contract():
    ...
    # Four-feed contract: DAY_1 (day gainers), MIN_5 (five-minute movers), VOLUME (absolute volume), RELATIVE_VOLUME_10D (rvol)
    assert set(calls)=={"DAY_1","MIN_5","VOLUME","RELATIVE_VOLUME_10D"}
    assert calls["DAY_1"]["sort_by"]=="CHANGE_RATIO"
    assert calls["MIN_5"]["sort_by"]=="CHANGE_RATIO"
