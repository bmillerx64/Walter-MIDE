...
def _continuation_reignition_st_pass(
    ...
    *,
    ...
    """Bounded continuation alternative to a fresh discrete ST flip.

    This path exists for already-established trends that base/re-accelerate after
    the original flip is stale. It is intentionally stricter than the ordinary
    fresh-flip path and can satisfy only the SuperTrend trigger lock.

    Required simultaneously:
    - current SuperTrend bullish;
    - at least 3 ordered confirmed rungs through 30s/1m/3m/5m with no conflict;
    - participation surge score >= 70;
    - expansion quality >= 60;
    - price no more than +1.5% above VWAP and not below the normal VWAP floor;
    - fresh 3m volume OR dollar-flow acceleration >= 1.5x.
    """
