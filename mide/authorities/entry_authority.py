"""Authoritative Walter Next Entry Authority.

This module now owns the canonical meaning of Entry Ready. The validated GS528
behavior is preserved exactly, but its meaning no longer lives in a numbered patch
module. GS528 remains only as a compatibility import/install shim while callers
migrate to this authority.

Entry Ready means and only means qualified_for_entry is True. This module does
not relax any qualification predicate, threshold, anti-chase rule, retest rule,
execution behavior, or order behavior.
"""
...
def entry_contract(record: dict) -> dict:
    ...
    for check in checks:
        if not check.get("passed"):
            blockers.append(
                str(
                    check.get("failed_reason")
                    or check.get("condition")
                    or "Trigger condition not passed"
                )
            )
...
def install() -> None:
    """Bind canonical entry meaning at the existing Opportunity State boundary."""
