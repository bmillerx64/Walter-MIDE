...
# ---------------------------------------------------------------------------
# GS479 deterministic headline catalyst magnitude
# ---------------------------------------------------------------------------

HEADLINE_MAGNITUDE_AUTHORITY = "CATALYST_CONTEXT_ONLY"
_HEADLINE_MAGNITUDE_ANALYZE_OWNER = "_walter_gs479_headline_magnitude_analyze_owner"
_HEADLINE_MAGNITUDE_CLASSIFY_OWNER = "_walter_gs479_headline_magnitude_classify_owner"
...
_HEADLINE_WORD_MONEY_RE = re.compile(
    r"\b(?P<value>\d+(?:\.\d+)?)\s*"
    r"(?P<unit>trillion|billion|million|bln|bn|mln|mm)\s*"
    r"(?:dollars?|usd)\b",
    re.IGNORECASE,
)
_HEADLINE_DURATION_RE = re.compile(
    r"\b(?P<value>\d+(?:\.\d+)?)\s*(?:-|\s)?\s*(?:years?|yrs?)\b",
    re.IGNORECASE,
)
_HEADLINE_CONTRACT_TERMS = (
    "agreement",
    "contract",
    "purchase order",
    "award",
...
def _headline_money_value(match: re.Match) -> float | None:
    try:
        value = float(match.group("value"))
    except (TypeError, ValueError):
        return None
    unit = str(match.group("unit") or "").casefold()
    multiplier = _HEADLINE_UNIT_MULTIPLIER.get(unit)
    if multiplier is None:
        return None
    return value * multiplier


def _headline_dollar_amounts(headline: str) -> list[float]:
    text = str(headline or "")
    values = []
    spans = []
    for match in _HEADLINE_MONEY_RE.finditer(text):
        amount = _headline_money_value(match)
        if amount is not None and amount > 0:
            values.append(amount)
            spans.append(match.span())
    for match in _HEADLINE_WORD_MONEY_RE.finditer(text):
...
def _headline_duration_years(headline: str) -> float | None:
    values = []
    for match in _HEADLINE_DURATION_RE.finditer(str(headline or "")):
        try:
            value = float(match.group("value"))
        except (TypeError, ValueError):
            continue
...
def _headline_scale_band(largest: float | None) -> str:
    if largest is None:
        return "UNDISCLOSED"
...
def _headline_format_money(value: float | None) -> str | None:
    if value is None:
        return None
...
def headline_magnitude_context(headline: str) -> dict:
    """Extract factual headline scale without inferring company-relative value."""
    text = " ".join(str(headline or "").split())
    normalized = text.casefold()
    amounts = _headline_dollar_amounts(text)
    largest = amounts[0] if amounts else None
    years = _headline_duration_years(text)
    contractual = any(term in normalized for term in _HEADLINE_CONTRACT_TERMS)

    facts = []
    if largest is not None:
        facts.append(f"{_headline_format_money(largest)} stated")
    if years is not None:
        facts.append(f"{years:g}-year term")

    return {
        "authority": HEADLINE_MAGNITUDE_AUTHORITY,
        "headline_scale_band": _headline_scale_band(largest),
...
def install_headline_magnitude_metadata() -> None:
    """Attach GS479 magnitude facts to GS315 metadata without changing classification."""
    from mide import gs315_news_intelligence as gs315

    current = gs315.classify_news_intelligence
    if getattr(current, _HEADLINE_MAGNITUDE_CLASSIFY_OWNER, False):
        return
    ...
    def classify_with_magnitude(headline: str, **kwargs):
        ...
        result["catalyst_magnitude"] = gs479.headline_magnitude_context(headline)
        return result
    ...
    setattr(
        classify_with_magnitude,
        _HEADLINE_MAGNITUDE_CLASSIFY_OWNER,
        True,
    )
    gs315.classify_news_intelligence = classify_with_magnitude


def install_headline_magnitude_records() -> None:
    """Attach copy-only GS479 magnitude context to analyzed candidate records."""
    from mide import discovery

    current = discovery.analyze_candidates
    if getattr(current, _HEADLINE_MAGNITUDE_ANALYZE_OWNER, False):
        return

    @wraps(current)
    def analyze_with_catalyst_magnitude(
        client,
        candidates,
        news_index,
        discovery_reasons,
        ...
        for record in records or []:
            ...
            if not headline:
                ...
                continue
            row = deepcopy(record)
            row["catalyst_magnitude"] = gs479.headline_magnitude_context(headline)
            enriched.append(row)
        ...
        if isinstance(diagnostics, dict):
            diagnostics["gs479_headline_catalyst_magnitude"] = {
                "authority": HEADLINE_MAGNITUDE_AUTHORITY,
                "records_with_headline": sum(
                    bool(record.get("headline"))
                    for record in enriched
                ),
                "records_with_disclosed_dollar_scale": sum(
                    bool(
                        (record.get("catalyst_magnitude") or {}).get(
                            "largest_stated_dollar_amount"
                        )
                    )
                    for record in enriched
                ),
    ...
    setattr(
        analyze_with_catalyst_magnitude,
        _HEADLINE_MAGNITUDE_ANALYZE_OWNER,
        True,
    )
    discovery.analyze_candidates = analyze_with_catalyst_magnitude


def install_headline_catalyst_magnitude_news() -> None:
    """Install all GS479 Discovery + News responsibilities in historical order."""
    install_headline_magnitude_metadata()
    install_headline_magnitude_records()
...
__all__ = [
    "install_headline_catalyst_magnitude_news",
    "install_headline_magnitude_records",
    "install_headline_magnitude_metadata",
    "headline_magnitude_context",
    "HEADLINE_MAGNITUDE_AUTHORITY",
