...
class WebullOpenAPIClient:
    ...
    def snapshots(self, symbols: Iterable[str]) -> dict[str, dict]:
        ...
        for row in rows:
            ...
            if not symbol or price is None:
                continue
            normalized[symbol] = {
                "latestTrade": {"p": price, "t": row.get("last_trade_time") or row.get("timestamp") or row.get("time")},
                "latestQuote": {"bp": _number(row.get("bid") or row.get("bid_price")), "ap": _number(row.get("ask") or row.get("ask_price"))},
                "dailyBar": {"c": price, "v": _number(row.get("volume") or row.get("total_volume")), "h": _number(row.get("high")), "l": _number(row.get("low"))},
                "prevDailyBar": {"c": _number(row.get("pre_close") or row.get("prev_close") or row.get("previous_close")), "v": _number(row.get("prev_volume"))},
    ...
    def bars(self, symbols: Iterable[str], *, start: datetime, timeframe="1Min",
        ...
        wanted = list(dict.fromkeys(str(symbol).strip().upper() for symbol in symbols if str(symbol).strip()))
        ...
        def normalize(rows):
            return [{"t": row.get("timestamp") or row.get("time") or row.get("t"),
                     ...
                     "v": _number(row.get("volume") or row.get("v"))} for row in rows]

        def single(symbol, reason):
            LOGGER.warning("WEBULL single-symbol history symbol=%s reason=%s", symbol, reason)
        ...
        def grouped(payload):
            value = payload.get("data", payload.get("result", payload)) if isinstance(payload, dict) else payload
            groups = {}
            if isinstance(value, dict):
                ...
                for item in value:
                    if not isinstance(item, dict):
                        continue
                    symbol = str(item.get("symbol") or item.get("ticker") or item.get("ticker_symbol") or "").upper()
        ...
        def fallback(batch, reason):
            ...
            LOGGER.warning("WEBULL batch history fallback batch_size=%d reason=%s", len(batch), reason)
            for symbol in batch:
        ...
        def request_batch(batch):
            ...
            try:
                ...
                payload = self.sdk.batch_bars(symbols=batch, category="US_STOCK", interval=interval,
                                              start_time=start.isoformat(), count=min(int(limit), 10_000),
                                              end_time=end.isoformat() if end else None,
                                              extend_hour_required=True, include_overnight=True)
                ...
                if _invalid_history_symbol(exc) and len(batch) > 1:
                    ...
                    LOGGER.warning("WEBULL batch history failed batch_size=%d reason=fallback_invalid_symbol; isolating invalid symbol", len(batch))
                    request_batch(batch[:midpoint])
                    request_batch(batch[midpoint:])
                else:
                    reason = "fallback_invalid_symbol" if _invalid_history_symbol(exc) else "fallback_batch_error"
                    fallback(batch, reason)
                    batch_fallback_count = len(batch)
            finally:
                LOGGER.warning("WEBULL batch history batch_size=%d elapsed_seconds=%.3f returned_symbols=%d returned_bars=%d fallback_count=%d",
                               len(batch), time.monotonic() - batch_started, returned_count,
                               sum(len(output.get(symbol, ())) for symbol in batch), batch_fallback_count)

        if len(wanted) == 1 and not force_batch:
            started = time.monotonic()
            single(wanted[0], call_reason)
            LOGGER.warning("WEBULL history complete total_symbols=1 count_per_symbol=%d batches=1 concurrency=1 elapsed_seconds=%.3f returned_symbols=%d returned_bars=%d fallback_count=0",
                           min(int(limit), 1200), time.monotonic() - started, len(output), sum(len(rows) for rows in output.values()))
        elif wanted:
            started = time.monotonic()
            batches = [wanted[offset:offset + WEBULL_HISTORY_BATCH_MAX] for offset in range(0, len(wanted), WEBULL_HISTORY_BATCH_MAX)]
            concurrency = min(WEBULL_HISTORY_MAX_CONCURRENCY, len(batches))
            with ThreadPoolExecutor(max_workers=concurrency, thread_name_prefix="webull-history") as executor:
                futures = [executor.submit(request_batch, batch) for batch in batches]
                for future in as_completed(futures):
                    future.result()
            LOGGER.warning("WEBULL batch history complete total_symbols=%d count_per_symbol=%d batches=%d concurrency=%d elapsed_seconds=%.3f returned_symbols=%d returned_bars=%d fallback_count=%d",
                           len(wanted), min(int(limit), 1200), len(batches), concurrency,
                           time.monotonic() - started, len(output), sum(len(rows) for rows in output.values()), fallback_count)
...
class LiveWebullProvider(WebullProvider):
    ...
    def __init__(self, app_key: str, app_secret: str, *, fallback=None, bootstrap=None,
        ...
        LOGGER.info("LiveWebullProvider initialization started streaming_enabled=%s rest_client_injected=%s sdk_client_injected=%s",
        ...
        self.diagnostics["webull_stream"] = {
            ...
            "stream_bypass_reason": (None if enable_streaming else "Streaming is optional and disabled until REST snapshots are proven"),
            "subscribed_symbols": 0, "cached_symbols": 0, "messages_received": 0,
            "last_message_timestamp": None, "stream_latency_ms": None,
            "subscription_failures": [], "disconnect_count": 0,
            "symbols_missing_prices": 0, "discovered_symbols": 0,
            "cached_snapshot_symbols": 0, "cached_snapshot_loaded": False,
            "snapshot_rest_succeeded": None,
        }
        self.diagnostics["market_data_sources"] = {
            "universe_provider": "Alpaca Trading API", "quote_provider": "Webull OpenAPI SDK",
            "bars_provider": "Webull OpenAPI SDK", "streaming_provider": "Webull OpenAPI SDK",
        }
        self._bootstrap = bootstrap
        self._snapshot_client = rest_client or WebullOpenAPIClient(app_key, app_secret, sdk_client=sdk_client,
    ...
    def pipeline_sources(self) -> list[dict[str, str]]:
        return [
            {"Stage": "Universe (tradable symbol list)", "Actual provider": "Alpaca Trading API",
             "Endpoint / operation": "GET /v2/assets (symbol master only)",
             "Code path": "build_seed_symbols → LiveWebullProvider.assets → AlpacaProvider.assets", "Alpaca used": "Yes — symbol master only"},
            {"Stage": "Quote / snapshot retrieval", "Actual provider": "Webull OpenAPI SDK",
             "Endpoint / operation": SNAPSHOT_OPERATION + (" (≤100 symbols; US_STOCK; extended/overnight explicitly enabled)" if self._extended_hours_enabled else " (≤100 symbols; US_STOCK; regular session)"),
             "Code path": "app._run_live_pipeline.<locals>.discover → LiveWebullProvider.initialize_quotes → WebullOpenAPIClient.snapshots; then LiveWebullProvider.snapshots reads the Webull cache", "Alpaca used": "No"},
            {"Stage": "Streaming quotes", "Actual provider": "Webull OpenAPI SDK", "Endpoint / operation": f"SDK MQTT subscribe via {STREAM_HOST}",
             "Code path": "LiveWebullProvider.ensure_stream → official SDK market-data stream", "Alpaca used": "No"},
            {"Stage": "News", "Actual provider": "None (provider abstraction)", "Endpoint / operation": "Webull OpenAPI has News Summary but no raw article feed; no external call",
             "Code path": "NewsService → UnavailableNewsProvider", "Alpaca used": "No"},
            {"Stage": "VWAP / volume calculations", "Actual provider": "Webull OpenAPI SDK + Walter local calculations", "Endpoint / operation": "SDK stock bars; session_vwap and volume metrics run locally",
             "Code path": "analyze_candidates → LiveWebullProvider.bars → WebullOpenAPIClient.bars", "Alpaca used": "No"},
            {"Stage": "Scanning / filtering", "Actual provider": "Walter local pipeline (using the inputs above)", "Endpoint / operation": "No provider endpoint; in-process gates, scoring, ranking, and filtering",
             "Code path": "WalterArchitectureV1.run → discovery.prefilter_snapshots / analyze_candidates → scanner_v2 and scoring → trader_priority_sort_key", "Alpaca used": "No"},
        ]
    ...
    @staticmethod
    def bars_frame(rows):
        """Build a sorted UTC OHLCV frame without dateutil's per-row slow path."""
        import pandas as pd
        ...
        if non_null.any() and numeric[non_null].notna().all():
            ...
            # Webull can return mixed ISO-8601 forms. format='mixed' keeps parsing
            # vectorized instead of falling back to dateutil once per row.
            parsed = pd.to_datetime(timestamps, utc=True, format="mixed", errors="coerce")
    ...
    def _on_event(self, event: MarketEvent) -> None:
        ...
        value = CachedMarketData(float(raw["price"]), _number(raw.get("volume")), _number(raw.get("bid")), _number(raw.get("ask")), event.source_timestamp_ms, now_ms)
    ...
    def ensure_stream(self, symbols: Iterable[str]) -> bool:
        ...
        try:
            ...
            LOGGER.error("WEBULL stream initialization failed; entering snapshot-only mode cached_snapshot_symbols=%s error=%s", len(self._snapshot_cache), exc)
            return False
    ...
    def initialize_quotes(self, symbols: Iterable[str], *, batch_size: int = MAX_SNAPSHOT_SYMBOLS) -> dict[str, float]:
        ...
        LOGGER.info("WEBULL universe before snapshot discovered_symbols=%s supported_symbols=%s rejected_symbols=%s", len(submitted), len(wanted), len(rejected))
        rest_succeeded = True
            ...
            try:
                ...
                if invalid:
                    for symbol in invalid:
                        ...
                        self.warnings.append(f"Skipped unsupported Webull snapshot symbol {symbol}: Webull returned HTTP 417 INVALID_SYMBOL")
                    remaining = [symbol for symbol in batch if symbol not in identified]
                    LOGGER.warning("WEBULL snapshot rejected invalid symbols symbols=%s batch_size=%d retry_count=%d", invalid, len(batch), int(bool(remaining)))
        ...
        for offset in range(0, len(wanted), batch_size):
            ...
            try:
                ...
                LOGGER.error("WEBULL snapshot REST failed independently of streaming batch_offset=%s batch_symbols=%s error=%s", offset, len(batch), exc)
                raise
            else:
                LOGGER.info("WEBULL snapshot REST succeeded independently of streaming batch_offset=%s requested_symbols=%s returned_symbols=%s", offset, len(batch), len(snapshots))
            snapshot_rows_decoded += int(getattr(self._snapshot_client, "last_snapshot_rows_decoded", len(snapshots)))
            snapshot_rows_normalized += int(getattr(self._snapshot_client, "last_snapshot_rows_normalized", len(snapshots)))
            now_ms = time.time_ns() // 1_000_000
            with self._lock:
                for symbol, snapshot in snapshots.items():
                    trade, daily, quote = snapshot.get("latestTrade") or {}, snapshot.get("dailyBar") or {}, snapshot.get("latestQuote") or {}
                    ...
                    if timestamp > 10_000_000_000_000:
                        timestamp //= 1_000_000
                    self.cache[symbol] = CachedMarketData(price, _number(daily.get("v")), _number(quote.get("bp")), _number(quote.get("ap")), timestamp, now_ms)
        ...
        LOGGER.info("WEBULL snapshot initialization summary discovered_symbols=%s snapshot_rows_decoded=%s snapshot_rows_normalized=%s cached_snapshot_symbols=%s symbols_missing_prices=%s",
                    len(submitted), snapshot_rows_decoded, snapshot_rows_normalized, len(self._snapshot_cache), d["symbols_missing_prices"])
        if not self._enable_streaming:
            LOGGER.info("WEBULL streaming bypassed after snapshot proof; cached_symbols=%s", len(self.cache))
    ...
    def snapshots(self, symbols: Iterable[str]) -> dict:
        ...
        with self._lock:
            snapshots = {symbol: dict(self._snapshot_cache.get(symbol, {})) for symbol in symbols if symbol in self.cache}
            cached_snapshot_count = sum(1 for symbol in symbols if symbol in self._snapshot_cache)
        d = self.diagnostics["webull_stream"]
        d["cached_snapshot_loaded"] = cached_snapshot_count > 0
        d["cached_snapshot_symbols"] = len(self._snapshot_cache)
        LOGGER.info("WEBULL cached snapshot load requested_symbols=%s loaded=%s loaded_symbols=%s cached_snapshot_symbols=%s",
                    len(symbols), d["cached_snapshot_loaded"], cached_snapshot_count, len(self._snapshot_cache))
        with self._lock:
        ...
        for symbol, live in cached.items():
            ...
            snapshot["latestTrade"] = {**snapshot.get("latestTrade", {}), "p": live.price, "t": live.source_timestamp_ms * 1_000_000}
            if live.volume is not None:
                snapshot["dailyBar"] = {**snapshot.get("dailyBar", {}), "v": live.volume, "c": live.price}
            if live.bid is not None or live.ask is not None:
                snapshot["latestQuote"] = {**snapshot.get("latestQuote", {}), "bp": live.bid, "ap": live.ask}
