...
GS473 reuses it after those boundaries to make fresh multi-timeframe WATCH NOW
attention audible without changing entry/alert qualification or anti-chase authority.
...
Safety contract:
- GS384/386/388/389/390/392/469/470/473 remain presentation/provenance/lifecycle/evidence only;
...
def install() -> None:
    ...
    # GS473 then patches the escalation phrase before app.py binds that callable by
    # name. They all run before the GS384 idempotence return so warm reloads cannot
    # miss the current infrastructure/audio boundary merely because GS384 is present.
    install_gs386()
    install_gs388()
    install_gs389()
    install_gs390()
