"""Walter dynamic conviction explanations.

Conviction is presentation-only. It consumes completed scanner decisions and
never qualifies, promotes, triggers, or ranks a candidate. Entry timing is also
observational: it describes whether an already-ranked candidate is actionable.
"""
...
CONVICTION_WEIGHTS = {
    "participation": 30.0, "dollar_flow": 20.0, "trend": 20.0,
    "structure": 15.0, "catalyst": 10.0, "opportunity": 5.0,
}
...
def calculate_conviction(record: dict, prior: dict | None = None) -> dict:
    ...
    participation_quality = _clamp(45.0 + participation_change * 1.5 + acceleration_change + volume_change * 0.35)

    dollar_change = _change(_number(record, "dollar_volume"), _number(prior, "dollar_volume"))
    ...
    structure_quality = 45.0 + (25.0 if relation == "above" else 5.0 if relation == "testing" else -25.0) + (20.0 if record.get("higher_lows") else 0.0)
    ...
    qualities = {
        "participation": participation_quality, "dollar_flow": dollar_quality,
        "trend": trend_quality, "structure": structure_quality,
        "catalyst": catalyst_quality, "opportunity": _clamp(_number(record, "opportunity_score_v2")),
    }
    components = {name: _points(q, CONVICTION_WEIGHTS[name]) for name, q in qualities.items()}
    ...
    watching = [
        ...
        {"label": "Entry timing confirmed", "complete": entry["entry_actionable"]},
    ]
    ...
    return {
        "conviction_v2_score": score, "conviction_trend": trend,
        "conviction_delta": delta, "conviction_history": history,
        ...
        "walter_take": " ".join([positive, middle, entry["entry_state_reason"]]),
        **entry,
    }
