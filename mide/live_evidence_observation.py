...
def live_evidence_observation(candidates: Iterable[Mapping[str, Any]], *, scan_timestamp: datetime | str | None = None, max_age_seconds: float = 120.0) -> dict[str, object]:
    ...
    for candidate in candidates:
        ...
        observations.append({
            "symbol": evidence["symbol"], "evidence_status": evidence["status"], "trusted": evidence["trusted"],
            "completeness_pct": evidence["completeness_pct"], "fresh": evidence["fresh"],
            "source_bar_age_seconds": evidence["source_bar_age_seconds"], "missing_fields": list(evidence["missing_fields"]),
            "coherence_failures": list(evidence["coherence_failures"]), "elevated": elevated,
    ...
    mismatches = [str(row["symbol"]) for row in observations if row["elevated"] and row["evidence_status"] != "TRUSTED"]
    return {
        "candidates_audited": audited, "trusted_count": trusted, "caution_count": caution,
        "insufficient_count": insufficient, "trusted_pct": round(trusted / audited * 100, 1) if audited else None,
        ...
        "nontrusted_elevated_count": len(mismatches), "nontrusted_elevated_symbols": mismatches, "observations": observations,
    }
...
def live_evidence_summary(report: Mapping[str, Any]) -> str:
    ...
    return (f"Evidence audit: {int(report.get('candidates_audited', 0) or 0)} candidates"
...
def render_live_evidence_diagnostics(ui: Any, report: Mapping[str, Any], *, readiness_report: Mapping[str, Any] | None = None) -> None:
    """Render Diagnostics from the exact completed-scan evidence/readiness snapshot.

    GS247 prefers the readiness report snapshotted by CompletedScan. The fallback
    exists only for callers/tests that have an evidence report without a completed
    scan; production Diagnostics passes the stored snapshot explicitly.
    """
    readiness = readiness_report if readiness_report is not None else evidence_readiness_report(report)
    ...
    mismatches = {row.get("symbol"): row for row in report.get("observations", []) if row.get("elevated") and row.get("evidence_status") != "TRUSTED"}
